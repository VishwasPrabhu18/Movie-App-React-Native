    ...
    <View className='bg-primary flex-1'>
      {loading ? (
        <View className='flex-1 items-center justify-center'>
          <Text className='text-light-200 font-semibold text-lg'>Loading...</Text>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
          ...
          <View className='flex-col items-start justify-center mt-5 px-5'>
            ...
            <MovieInfo label='Genres' value={movie?.genres?.map((genre: any) => genre.name).join(' - ')} />
            <View className='flex flex-row justify-between w-1/2'>
              <MovieInfo label='Budget' value={`$${movie?.budget! / 1_000_000} million`} />
              <MovieInfo label='Revenue' value={`$${Math.round(movie?.revenue! / 1_000_000) ?? 0}`} />
            </View>
